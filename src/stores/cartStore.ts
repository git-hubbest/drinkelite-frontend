  ...
  persist(
    (set, get) => ({
      ...
      addItem: async (item) => {
        ...
        // For subscriptions, treat same variant + different sellingPlanId as different items
        const matchKey = (i: CartItem) => i.variantId === item.variantId && (i.sellingPlanId || null) === (item.sellingPlanId || null);
        const existingItem = items.find(matchKey);
        set({ isLoading: true });
        try {
          if (!cartId) {
            ...
            if (result.success) {
              set({ items: get().items.map(i => matchKey(i) ? { ...i, quantity: newQuantity } : i) });
          ...
          set({ isLoading: false, lastAddedItem: get().items.find(i => i.variantId === item.variantId) ?? null, isCartOpen: true });
        }
      },
      ...
      closeCart: () => set({ isCartOpen: false, lastAddedItem: null }),
